...
# Dicionários de formatação em português
MESES_PT = {
...
# ==========================================================
# 1. TEMA E CSS (ESTILO FINAL TRAVADO)
# ==========================================================
...
# CSS para o tema e para o estilo de observações (cabeçalho vermelho)
st.markdown("""
<style>
    ...
    /* Estilo do cabeçalho de observações */
    .obs-header {
...
# ==========================================================
# 2. MOTOR DE BANCO DE DADOS (GITHUB COM TTL)
# ==========================================================
@st.cache_data(ttl=5)
def carregar_bancos():
    ...
    try:
        ...
        # Carregar Usuários (e criar se não existir)
        try:
        ...
        # Carregar Observações (e criar se não existir)
        try:
            cont_o = repo.get_contents("observacoes.csv")
            df_o = pd.read_csv(io.StringIO(cont_o.decoded_content.decode()))
            sha_o = cont_o.sha
        except:
            # Colunas: Data (chave), Observacao
            df_o = pd.DataFrame(columns=["Data", "Observacao"])
            sha_o = None
